export function buildEmailHtml(appUrl: string, domain: string, orderId: string): string {
  const reportUrl = `${appUrl ?? 'https://emailcorreto.com.br'}/relatorio?order=${orderId}`;
